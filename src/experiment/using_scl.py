...
#likes[x] > 1000 and x.split('_')[0] == korea)

shares = filter(lambda x : x > 90,map(lambda x : result_dict[x][0],result_dict))
comments = filter(lambda x : x > 90,map(lambda x : result_dict[x][1],result_dict))
likes = filter(lambda x : x > 1180,map(lambda x : result_dict[x][2],result_dict))
